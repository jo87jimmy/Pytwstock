...
def insert_income(ID_NO):
    url = """https://goodinfo.tw/StockInfo/StockFinDetail.asp?RPT_CAT=IS_M_QUAR_ACC&STOCK_ID="""+ID_NO+""""""
    url_headers = {
    ...
    table_name = {
        'income_operating_profit': '本業獲利',
        'income_Non_operating_income': '業外損益',
        'income_net_profit': '淨損益',
    }
    ...
    for x in range(row_count):
        ...
        for col in table.iloc[x]:
            if col in table_name.values():  # insert尋找寫入時,對應的資料表
                table_temp = list(table_name.keys())[
                    list(table_name.values()).index(col)]
