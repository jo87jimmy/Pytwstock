...
def insert_balance(ID_NO):
    url = """https://goodinfo.tw/StockInfo/StockFinDetail.asp?RPT_CAT=BS_M_QUAR&STOCK_ID="""+ID_NO+""""""  # +stock_no
    url_headers = {
    ...
    #BeautifulSoup 的資料整理寫法
    # x = 0
    # for detail in items.contents:

    #     for detail in items.contents[x].contents[0]:  # 讀項目
    #         if detail == ' ':
    #             break
    #         val = detail.string.strip()
    #         if val == '':
    #             break
    #         if (val == '金額') or (val == '％'):
    #             continue
    ...
    #             tag_table = 'Debt'
    #         elif val == '股東權益':
    #             tag_table ='Shareholder_equity'
    #         else:    
    #              print(val) 
                    
    #         y = 1

    #         for i in range(1, items.contents[x].__len__()):  # 讀取值
    #             if items.contents[x].contents[i] == ' ':
    #                 break
    #             val2 = items.contents[x].contents[i].string.strip()
    #             if val2 == '':
    #                 break
    #             if (val2 == '金額') or (val2 == '％'):
    #                 continue
